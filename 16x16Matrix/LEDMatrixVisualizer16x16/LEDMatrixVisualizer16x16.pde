...
2, 8x16 LED Matrix Backpacks, which is produced by Adafruit Industries
...
void printFrame(){
  ...
  for (int i = 0; i < hLEDs; i++) {
    ...
    for(int j = 8; j < 16; j++){
      ...
      else if(j == 15 && i == hLEDs-1) println(LEDPixels[j][i]);
      else if (j == 15) println(LEDPixels[j][i] + ", ");
      else print(LEDPixels[j][i]);
    }
  }
  println("};");
}
