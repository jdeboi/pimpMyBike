...
///////////////////////////
void HT1632LEDMatrix::clearLEDs() {
...
void HT1632LEDMatrix::setLEDs() {
    for (int h = 0; h < _height; h++) {
        for (int w = 0; w < _width; w++) {
            drawPixel(w, h, LEDs[w][h]);
        }
    }
}    

uint8_t HT1632LEDMatrix::getLEDValue(int x, int y) {
    return LEDs[x][y];
} 
    
uint8_t HT1632LEDMatrix::getLEDs() {
    return LEDs;
}
    
void HT1632LEDMatrix::translate(uint8_t d) {
    //move to the right
    if (d==1) {
